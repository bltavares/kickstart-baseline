...
baseline.ubuntu.add_universe_package_source() {
  ...
  [ -z "$code_name" ] && kickstart.info "Couldn't fetch Ubuntu's codename" && exit 1

  kickstart.info "Check for presence of universe or add it"
  kickstart.file.contains /etc/apt/sources.list "$code_name universe" || ( echo deb http://archive.ubuntu.com/ubuntu $code_name universe >> /etc/apt/sources.list )
